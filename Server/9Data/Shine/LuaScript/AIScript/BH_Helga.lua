...
cExecCheck "Invisible_Init"
    ...
    local mobtable = {}
	for i = 1, HGBox_ChestsSpawns do
		local box = BH_HelgaBox[((i - 1) % #BH_HelgaBox) + 1]
    ...
    local randomnumbers = {}
    for i = 1, #BH_HelgaBox do
        local num = math.random(1, #mobtable)
        if randomnumbers[num] == nil then
            randomnumbers[num] = true
            cSetItemDropMobID( mobtable[num], BH_HelgaBox[i]["ItemDropMobIndex"] )
        else
            i = i - 1
        end
    end
	...
	Var.Wait = {}
	Var.Wait.Second = cCurSec() + HGBox_VanishTime
...
cExecCheck "Invisible_AllVanish"
	...
	cRegenGroupActiv("BH_Helga", "BH_HelgaBox", 0)
	Common_RemoveTreasure(Var.MapIndex)
