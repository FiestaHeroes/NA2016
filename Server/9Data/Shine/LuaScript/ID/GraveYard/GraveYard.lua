...
require( "ID/GraveYard/Data/Name" )				-- 파일경로, 파일이름, 역참조, 단계 진행을 위한 네임 테이블
require( "ID/GraveYard/Data/Chat" )				-- 모든 채팅 관련
require( "ID/GraveYard/Data/Process" )			-- 각종 딜레이타임과 링크 정보, 퀘스트 등의 진행 관련 데이터
require( "ID/GraveYard/Data/Regen" )			-- 리젠 데이터(그룹, 몹, NPC, 문, 아이템 등의 리젠 종류, 위치 및 속성 관련)
require( "ID/GraveYard/Data/Stuff" )			-- Stuff관련 처리 데이터( NPC 액션 정보 및 액션을 일으키기 위한 요소 정보 )
require( "ID/GraveYard/Data/Boss" )				-- Boss관련 처리 데이터( 보스전 Phase Data(소환 몹 포함), 상태이상, 치프몹 관련 경고문 정보 )

require( "ID/GraveYard/Functions/SubFunc" )		-- 전체적인 진행에 필요한 각종 Sub Functions
require( "ID/GraveYard/Functions/Routine" )		-- 몹 등에 붙는 AI 관련 루틴들
require( "ID/GraveYard/Functions/Progress" )	-- 각 단계가 정의된 진행 함수들
--]]
...
cExecCheck "Main"
	...
	then
		...
		Var["GateProcess"]	= {}	-- 열린 게이트 정보를 담는 메모리
